use ark_bn254::Fq;
use gkr::multilinear_poly::MultilinearPolynomial;
...
fn test_partial_evaluate() {
    ...
    assert_eq!(
        ...
        MultilinearPolynomial::new(&[Fq::from(0), Fq::from(9), Fq::from(0), Fq::from(11)])
    );
}
...
fn test_polynomial_tensor_add() {
    ...
    let wb = MultilinearPolynomial::new(&[Fq::from(1), Fq::from(2)]);
    // w(c) = [3,4] (one variable)
    let wc = MultilinearPolynomial::new(&[Fq::from(3), Fq::from(4)]);
...
fn test_polynomial_tensor_mul() {
    // w(b) = [2,3]
    let w_b = MultilinearPolynomial::new(&[Fq::from(2), Fq::from(3)]);

    // w(c) = [4,5]
    let w_c = MultilinearPolynomial::new(&[Fq::from(4), Fq::from(5)]);
    ...
    // 3*5 = 15 (w_b[1] * w_c[1])
    let expected =
        MultilinearPolynomial::new(&[Fq::from(8), Fq::from(10), Fq::from(12), Fq::from(15)]);
...
fn test_polynomial_tensor_mul_different_lengths() {
    let w_b = MultilinearPolynomial::new(&[Fq::from(2), Fq::from(3)]);
    let w_c = MultilinearPolynomial::new(&[Fq::from(4)]); // Different length
